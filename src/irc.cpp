...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #BioCoinTEST2\r");
            Send(hSocket, "WHO #BioCoinTEST2\r");
        } else {
            // randomly join #BioCoin00-#BioCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #BioCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #BioCoin%02d\r", channel_number).c_str());
        }
