BioCoin-qt: Qt4 GUI for BioCoin
===============================
...
Alternatively, install Qt Creator and open the `BioCoin-qt.pro` file.

An executable named `BioCoin-qt` will be built.
...
A warning for people using the *static binary* version of BioCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of BioCoin is linked against libdb5.3.
...
installed causes BioCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
